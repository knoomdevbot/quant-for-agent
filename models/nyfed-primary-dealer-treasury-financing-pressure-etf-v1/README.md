...
Because the source/vintage gate failed first, no Alpaca/qfa ETF performance backtest was accepted for this scout.  The local environment also lacked `ALPACA_API_KEY` and `ALPACA_SECRET_KEY`, so no successful Alpaca OHLCV request occurred.  No CSV, daemon, or order path was used.
