"""Cost-aware low-volatility quality proxy for mega-cap equities.

Research-only qfa model for AR-029. Exposes generate_signals(context) ->
dict[symbol, weight]. Uses Alpaca/qfa OHLCV only; no order placement.
"""
...
class ModelParams:
    # Pre-specified AR-029 refined setting: keep the AR-010 126d low-vol +
    # positive momentum quality proxy, but evaluate ranks on a month-end anchor
    # to reduce needless daily turnover. Neighbor stability is tested in the
    # evaluation artifact; this file is the selected production research spec.
    vol_window = 126
    momentum_window = 126
...
def _month_end_anchor(close: pd.DataFrame, as_of: pd.Timestamp) -> pd.Timestamp:
    """Use the latest completed month-end observation unless today is month-end."""
    if close.empty:
        return as_of
    dates = pd.DatetimeIndex(close.index)
    naive_dates = dates.tz_convert(None) if dates.tz is not None else dates
    naive_asof = as_of.tz_convert(None) if as_of.tzinfo is not None else as_of
    current_month = naive_asof.to_period("M")
    month_index = naive_dates.to_period("M")
    prior_or_current = dates[month_index < current_month]
    # If this is the last observation in its month, allow current month anchor.
    same_month = dates[month_index == current_month]
    if len(same_month) and same_month[-1] == as_of:
        return as_of
    if len(prior_or_current):
...
def generate_signals(context):
    ...
    Signal construction:
    - Uses only symbols in the AR-029 mega-cap universe.
    - Applies a monthly rebalance proxy by computing ranks as of the latest
      completed month-end anchor available in context.prices.
    - Ranks by inverse 126-trading-day realized volatility.
    - Quality proxy: requires positive 126-day total return when enabled.
    - Selects top 6 names, inverse-vol weighted, gross <= 1.0, single-name cap 20%.
    """
    ...
    anchor = _month_end_anchor(close[tradable], as_of)
    close = close.loc[:anchor, tradable]
