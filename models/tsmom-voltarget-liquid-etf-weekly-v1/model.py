"""Volatility-targeted time-series momentum alpha for liquid ETFs.

qfa contract: expose generate_signals(context) -> dict[str, float].
The model is intentionally data-source agnostic and only reads context.prices.
"""
...
DEFAULT_PARAMS = {
    ...
    "target_vol": 0.10,
    "max_abs_weight": 0.35,
    # Require enough history for the medium-term trend and realized-vol estimate.
    # This avoids treating the first few bars of a backtest as a valid 126-day signal.
    "min_periods": 127,
}
...
def _cap_and_renormalize(weights: dict[str, float], cap: float) -> dict[str, float]:
    """Cap absolute concentration, preserving signs and gross-normalizing to 1."""
    if not weights:
        return weights
...
def generate_signals(context):
    """Return target portfolio weights by symbol.

    Signal:
    - compute trailing total return over lookback_days (or available history);
    - keep positive time-series momentum only;
    - scale each positive signal by target_vol / realized_vol;
    - gross-normalize and cap single-name concentration.

    The default parameters require a full 126-day lookback plus current bar before
    emitting signals; insufficient history returns all-zero weights.
    """
